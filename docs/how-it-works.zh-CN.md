...
点击「安全弹出」先进行只读预检。没有需要处理的对象时，直接尝试普通系统弹出；需要退出应用、停止限定服务或普通推出关联映像时，先展示影响，再由你确认。

- 进程名称或命令行匹配只表示「可能相关」，不能据此退出应用。
- 自动处理必须有目标卷或关联映像卷上的文件占用证据，并验证 PID、用户、启动时间和可执行文件身份。
- 检测超时、失败或不完整会明确显示；未知不会显示成正常或无人占用。
- 切盘、重新检测和挂载变化使旧任务失效，旧结果不能覆盖新盘。
- 「中止」会停止后续步骤，已经发出的退出或停止请求无法撤销。
- 系统弹出命令执行期间必须等待并核验结果；已明确拒绝后的两次尝试之间可以中止。未核验成功时不会提示可以拔线。
...
| 检测不完整 | 重新检测、取消，或明确选择「尝试普通系统弹出」 |
| 身份已核验的当前用户终端任务、未知后台任务 | 提供默认未勾选的终止选项，提醒任务可能中断 |
| 无法核验的进程 | 不操作进程，可尝试普通系统弹出 |
| 可写、只读或属性未知的映像 | 确认后尝试普通推出，包含未挂载但仍连接的映像 |
| 同一物理盘有多个挂载卷 | 列出关联卷，普通流程不自动清理进程；普通弹出失败后可单独确认强制 |
| 目标身份变化、不是外置物理盘或拓扑无法确认 | 停止流程，交给用户检查 |

「尝试普通系统弹出」不退出应用、不停止服务、不主动推出映像，不使用 force。检测不完整时，应用没有声称盘上无人占用。

确认后的顺序：确认目标与操作范围 → 请求 GUI 应用退出 → 停止限定服务 → 普通推出关联映像 → 复查 → 系统弹出 → 核验映像断开及关联卷卸载；固定外置设备可以仍保留在系统磁盘列表中。

### 哪些对象允许代为处理

- **GUI 应用**：必须映射到实际运行实例，并属于当前用户；通过 `NSRunningApplication.terminate()` 请求该实例退出。应用自行处理保存对话框；拒绝或仍持有文件时提供「继续检查并弹出」，不重复发送退出请求，不升级为强杀。
- **限定服务**：当前用户的 Gradle daemon、Kotlin daemon、adb server，有实际文件占用证据并确认后才收到 SIGTERM。发送成功不代表退出成功，会继续检查进程退出或文件占用释放；仅实际确认退出的进程计入停止数量。
- **其他任务**：身份已核验的当前用户终端任务、前台构建及未知后台进程，可以在逐项勾选并确认后发送 SIGTERM。默认不勾选，并提醒可能中断任务、丢失未保存进度；无法可靠识别的对象仍需手动处理。
- **系统及其他用户进程**：不操作。推断条目与实际扫描证据分开展示。
- **磁盘映像**：三种读写状态分别展示，确认后都可普通 detach；扫描包含映像内部卷，嵌套映像先处理子映像。命令返回后还要核实映像已断开。

预览超过 30 秒或目标挂载状态变化后需要重新核验。确认不授权新增对象；检测到新增占用或映像时返回更新后的预览，保留已完成操作的计数。

系统拒绝后，本地保留失败步骤、原始错误、阻塞 PID 和已完成动作，供查看或复制；刷新不会清除。新出现的用户进程必须重新核验文件证据并确认。仅明确的临时忙碌最多尝试三次；新增占用、权限错误、超时和结果未知均停止自动重试。

### 普通失败后的强制弹出

只有普通推出映像或弹出磁盘明确失败后，才出现「强制弹出…」。点击后重新只读检测，列出全部受影响卷和映像，再由用户确认「可能丢失未保存的数据或损坏磁盘映像」。该确认只用于本次操作，过期或范围扩大需要重新确认。

确认后依次强制 detach 关联映像、确认映像断开、`diskutil unmountDisk force`、普通 `diskutil eject`。不强杀应用或系统服务、不提权。卸载后按原卷 UUID、设备和物理盘关系复核目标，不能依赖已消失的挂载路径。已完成的映像推出和卷卸载不重复。

命令超时或状态不明时先进入「结果待确认」，重新检测只读取状态，不自动重复或升级命令。确认完成后显示「已强制弹出」，保留未保存数据完整性无法保证的提示；权限不足或设备故障仍可能失败。

## 检测能力与边界

普通权限只能检查可见句柄，完整扫描完成也不表示看到了所有用户和系统服务。`lsof +f -- <挂载点>` 直接查询卷上的文件句柄，不遍历受保护目录；仍保留 30 秒上限，并明确报告权限、超时和解析错误。不完整结果可以用于解释问题，不能当作自动处理的完整依据。
...
本次版本的验收记录见 [1.3.0 弹出恢复验收记录](validation-eject-recovery.md)；[1.1.0 验收记录](validation-1.1.0.md)保留早期安全流程的验证范围。
