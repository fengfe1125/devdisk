...
- 不会强杀程序或后台服务，也不会未经单独确认就强制弹出。
- 不会因为名字对得上就关掉一个程序，只处理它亲眼看到正在占用这块盘的。
- 没检查清楚，不会说「没人在用」或「可以拔了」。检查失败或超时，会直接告诉你。
- 不碰系统进程和其他用户的进程。
- 不要管理员权限。需要改系统设置时，只给你一条能复制的命令，不替你执行。

身份已核验的终端任务和后台任务也能逐项勾选终止，默认不勾选，并提醒可能中断工作；无法核验的对象仍需手动处理。应用等待保存时，处理后可以「继续检查并弹出」；系统拒绝时会保留阻塞 PID 和错误详情。

可写磁盘映像也能在确认后普通推出。普通推出或弹出失败后，可点击「强制弹出…」，核对受影响的映像和卷，并确认可能丢失未保存数据、损坏映像的风险。强制不会自动发生；超时先核对结果。完成后会明确标注是否使用过强制操作。

## 语言

在面板设置顶部，或设置窗口（⌘,）的「通用」页，选择 **跟随系统**、**简体中文** 或 **English**。切换立即同步到已打开的窗口，并保存供下次使用。默认跟随系统：系统首选语言为中文时显示简体中文，否则显示英文。系统权限弹窗的语言由 macOS 决定。
...
- [1.3.0 弹出恢复验收记录](docs/validation-eject-recovery.md)
