...
弹层里刻意不走 SwiftUI 的 `Settings` 场景——状态栏弹层内直接切到设置屏，⌘, 仍然打开标准设置窗口。
...
**8. 状态栏弹层必须锚在状态栏按钮上。** SwiftUI 的菜单栏 window 风格把尺寸和位置交给私有宿主窗口；宿主可能保留上一屏的高度，让短卡片垂直居中并露出透明边带。现在由 `StatusBarController` 持有原生 `NSStatusItem` 和 `NSPopover`，用状态栏按钮的 bounds 作为锚点并从 `.minY` 展开，弹层始终贴在菜单栏下方；`PopoverChrome` 负责清理宿主背景并绘制圆角卡片。

注意方向：让**内容**向内缩（负内边距）在顶部有效，但会把底部的版本行推出窗口下沿被裁掉——修外观不能以丢内容为代价。

**9. 短内容自然定高，长内容才滚动。** 给短内容套一个固定高度的 ScrollView，会让弹层一直保持最高一屏的尺寸；现在只有超过上限时才创建 ScrollView，并在屏幕切换时清空旧高度。`PopoverChrome` 同时用内容测量值更新宿主尺寸，避免短卡片被放进旧的高窗口里。
...
**快照验证覆盖不到弹层。** 弹层的 ScrollView 高度、NSPopover 的状态栏锚点、以及依赖应用活跃状态的 AppKit 动作，全都只能靠真机点开菜单栏图标验证——这三类问题各出过一次，都是快照和窗口路径漏掉的。
