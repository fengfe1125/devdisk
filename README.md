...
**8. `MenuBarExtra(.window)` 的宿主面板会露出来。** 它把内容装进一个比内容高 20pt、方角且不透明的面板里——上下各露一条灰带，四角是硬直角，弹层看着像「内容被塞进一个盒子」而不是一张卡片。做法是接管那个窗口：把窗口背景设为透明，由面板自己画圆角背景，并让**背景向外扩** 10pt 填掉宿主的内边距。
